#include <stdio.h>
int main(){
   // we should always write all the above things from intmain to  return 0;  cause its the structure of c  compulsory.
    char onecharacter='h';
// char is used to declare only one character inside cause it can only store one 
    char multiplecharacters[]= "hello to the characters";
...
//Use float when you need to conserve(it occupy 4bytes ) memory, and the precision of about 7 digits is sufficient.
     ...
     double Clasher=74.4; //widely used and should start to use it 
